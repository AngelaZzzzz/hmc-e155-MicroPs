...
//			registered, so it changes one clock after the counter crosses a quarter
//			boundary. With MAX_COUNT = 12 each row lasts STEP = 3 clocks. Checks the
//			order, the hold time, freezing while enable is low, and reset.
...
module keypad_scanner_tb();
	...
	initial begin
		...
		// Test 2: row[3] holds for STEP clocks plus the one-clock output register delay
		reset = 0;
		repeat (STEP) @(posedge clk); #1;
		assert (row == 4'b1000)
			$display("PASSED! Output holds at 1000 for the first row period at time: %0t.", $time);
		else
			$error("FAILED! Output changed to %b before the first row period ended at time: %0t.", row, $time);
